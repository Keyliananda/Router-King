...
class _FakeConnectedSender:
    def __init__(self, status=None, streaming=False, coordinate_lines=None):
        self.commands = []
...
class TestMainDock(unittest.TestCase):
    ...
    def _rectangle_template_controls(self):
        return {
            ...
            "final_surface": _DummyCheckBox(False),
            "final_surface_step": _DummySpin(0.2),
        }
    ...
    def test_send_jog_blocks_controller_outside_manual_xyz(self):
        main_dock = _load_main_dock_module()
        widget = main_dock.RouterKingDockWidget.__new__(main_dock.RouterKingDockWidget)
        ...
        result = widget._send_jog(x=0.5, feed=600, source="Controller", log=False)

        self.assertFalse(result)
        self.assertEqual(widget._controller_status.text, "Controller: locked - use Prepare Manual XYZ")
        widget._append_console.assert_not_called()
    ...
    def test_jog_z_down_allowed_near_positive_z_home_with_shifted_profile(self):
        main_dock = _load_main_dock_module()
        sender = _FakeConnectedSender(status={"state": "Idle", "MPos": "-267.338,-330.675,-2.100"})
        widget = main_dock.RouterKingDockWidget.__new__(main_dock.RouterKingDockWidget)
        widget._sender = sender
        widget._explore_active = False
        widget._limits = {"X": 400.0, "Y": 400.0, "Z": 60.0}
        ...
        with mock.patch.object(main_dock, "grbl_load_machine_profile", return_value=(profile, "/tmp/profile.json")):
            result = widget._send_jog(z=-1.0, feed=600, source="test")

        self.assertTrue(result)
        self.assertEqual(sender.commands, ["$J=G91 Z-1.000 F600"])
    ...
    def test_prepare_manual_xyz_enables_controller_without_motion(self):
        ...
        widget._manual_xyz_preview_origin_wpos = None
    ...
    def test_prepare_manual_xyz_blocks_without_reliable_live_position_preserves_last_position(self):
        main_dock = _load_main_dock_module()
        sender = _FakeConnectedSender(status={"state": "Idle"})
        widget = main_dock.RouterKingDockWidget.__new__(main_dock.RouterKingDockWidget)
        widget._sender = sender
        ...
        widget._manual_xyz_preview_origin_wpos = {"x": 1.0, "y": 2.0, "z": 3.0}
        widget._manual_xyz_prepare_mpos = {"x": 1.0, "y": 2.0, "z": 3.0}
        widget._manual_xyz_prepare_wpos = {"x": 1.0, "y": 2.0, "z": 3.0}
        widget._manual_xyz_work_origin_fallback = True
        widget._jog_work_origin_mpos = {"x": 1.0, "y": 2.0, "z": 3.0}
        widget._controller_enable = _DummyCheckBox(False)
        widget._controller_timer = _DummyTimer()
        ...
        self.assertEqual(sender.commands, [])
        self.assertFalse(widget._controller_manual_xyz_active)
        self.assertEqual(widget._manual_xyz_preview_wpos, {"x": 1.0, "y": 2.0, "z": 3.0})
        self.assertEqual(widget._manual_xyz_prepare_mpos, {"x": 1.0, "y": 2.0, "z": 3.0})
        self.assertFalse(widget._controller_timer.started)
        self.assertIn("reliable live position unavailable", widget._append_console.call_args_list[-1].args[0])
        self.assertIn("preserved", widget._append_console.call_args_list[-1].args[0])
        widget._request_status.assert_called_once_with()
    ...
    def test_manual_xyz_preview_uses_real_live_origin_on_prepare(self):
        main_dock = _load_main_dock_module()
        widget = main_dock.RouterKingDockWidget.__new__(main_dock.RouterKingDockWidget)
        widget._controller_manual_xyz_active = True
        widget._manual_xyz_preview_wpos = None
        ...
        self.assertFalse(widget._manual_xyz_work_origin_fallback)
    ...
    def test_manual_xyz_preview_derives_real_work_position_when_status_has_wco_without_wpos(self):
        main_dock = _load_main_dock_module()
        widget = main_dock.RouterKingDockWidget.__new__(main_dock.RouterKingDockWidget)
        widget._controller_manual_xyz_active = True
        widget._manual_xyz_preview_wpos = None
        ...
        self.assertFalse(widget._manual_xyz_work_origin_fallback)
        self.assertEqual(widget._manual_xyz_preview_origin_wpos, {"x": -297.0, "y": -377.0, "z": -4.0})
        self.assertEqual(widget._manual_xyz_preview_wpos, {"x": -287.0, "y": -372.0, "z": -4.0})

    def test_manual_xyz_preview_uses_profile_work_offset_when_status_has_only_mpos(self):
        main_dock = _load_main_dock_module()
        widget = main_dock.RouterKingDockWidget.__new__(main_dock.RouterKingDockWidget)
        widget._controller_manual_xyz_active = True
        widget._manual_xyz_preview_wpos = None
        ...
        profile = {"work_offset": {"x": 0.0, "y": 0.0, "z": 1.0}}
        with mock.patch.object(main_dock, "grbl_load_machine_profile", return_value=(profile, "/tmp/profile.json")):
            widget._prepare_manual_xyz_preview_baseline(prepare_status)
            widget._update_manual_xyz_preview_position(prepare_status, force=True)
            widget._update_manual_xyz_preview_position(moved_status, force=True)
        ...
        self.assertFalse(widget._manual_xyz_work_origin_fallback)
        self.assertEqual(widget._manual_xyz_preview_origin_wpos, {"x": -297.0, "y": -377.0, "z": -4.0})
        self.assertEqual(widget._manual_xyz_preview_wpos, {"x": -292.0, "y": -375.0, "z": -4.0})
    ...
    def test_manual_xyz_fit_selection_keeps_preview_on_fitted_gcode(self):
        main_dock = _load_main_dock_module()
        widget = main_dock.RouterKingDockWidget.__new__(main_dock.RouterKingDockWidget)
        widget._controller_manual_xyz_active = True
        widget._manual_xyz_preview_wpos = {"x": 999.0, "y": 999.0, "z": 0.0}
        widget._template_fit_candidates = [{"start_x": 0.0, "start_y": 0.0}]
        widget._template_fit_index = 0
        ...
        path = widget._preview_path_from_current_state(main_dock.rectangle_pocket(spec).gcode)
        first_cut = widget._first_cut_point(path)
        cut_start = widget._preview_cut_start_point(path)

        self.assertEqual((first_cut.x, first_cut.y), (1.0, 1.0))
        self.assertEqual((cut_start.x, cut_start.y), (1.0, 1.0))
    ...
    def test_preview_tool_work_position_uses_live_manual_xyz_z_position(self):
        main_dock = _load_main_dock_module()
        widget = main_dock.RouterKingDockWidget.__new__(main_dock.RouterKingDockWidget)
        widget._sender = _FakeConnectedSender(
            status={
                "MPos": "-397.000,-397.000,-8.000",
                "WPos": "0.000,0.000,-5.000",
                "WCO": "-397.000,-397.000,-3.000",
            }
        )
        widget._controller_manual_xyz_active = True
        widget._manual_xyz_preview_wpos = {"x": 12.0, "y": 18.0, "z": -5.0}
        widget._controller_guard_mpos = {"x": -385.0, "y": -379.0, "z": -8.0}
        widget._controller_guard_wpos = {"x": 12.0, "y": 18.0, "z": -5.0}

        self.assertEqual(widget._preview_tool_work_position(), {"x": 12.0, "y": 18.0, "z": -5.0})
        self.assertEqual(widget._preview_tool_machine_position(), {"x": -385.0, "y": -379.0, "z": -8.0})

    def test_preview_z_up_status_uses_shifted_positive_z_machine_limit(self):
        main_dock = _load_main_dock_module()
        widget = main_dock.RouterKingDockWidget.__new__(main_dock.RouterKingDockWidget)
        widget._sender = _FakeConnectedSender(
            status={
                "MPos": "-397.000,-397.000,-20.000",
                "WPos": "0.000,0.000,-17.000",
                "WCO": "-397.000,-397.000,-3.000",
            }
        )
        widget._controller_manual_xyz_active = False
        ...
        profile = {
            "machine_limits": {"x": [-397.0, 3.0], "y": [-397.0, 3.0], "z": [-63.0, -3.0]},
            "settings": {"$23": "3", "$27": "3"},
            "prefer_profile_limits": True,
        }

        with mock.patch.object(main_dock, "grbl_load_machine_profile", return_value=(profile, "/tmp/profile.json")):
            z_status = widget._preview_z_up_status()

        self.assertEqual(z_status["current_mpos_z"], -20.0)
        self.assertEqual(z_status["target_mpos_z"], -3.0)
        self.assertEqual(z_status["target_wpos_z"], 0.0)
        self.assertEqual(z_status["remaining_mm"], 17.0)

    def test_machine_status_text_reports_remaining_z_up_travel(self):
        main_dock = _load_main_dock_module()
        widget = main_dock.RouterKingDockWidget.__new__(main_dock.RouterKingDockWidget)
        status = {
            "state": "Idle",
            "MPos": "-397.000,-397.000,-8.500",
            "WPos": "0.000,0.000,-5.500",
            "WCO": "-397.000,-397.000,-3.000",
        }
        ...
        with mock.patch.object(main_dock, "grbl_load_machine_profile", return_value=(profile, "/tmp/profile.json")):
            text = widget._machine_status_text(status)

        self.assertEqual(text, "Machine: Idle | Pos: 0.000,0.000,-5.500 | Z up: 5.5 mm")
    ...
    def test_air_run_generates_current_manual_template_before_streaming(self):
        ...
        with mock.patch.object(main_dock, "grbl_load_machine_profile", return_value=({"settings": {}}, "/tmp/profile.json")):
            with mock.patch.object(
                main_dock,
                "grbl_validate_gcode",
                return_value={"valid": True, "line_count": 10, "bounding_box": {"x": [10, 50], "y": [20, 50], "z": [1.5, 1.5]}},
            ):
                widget._on_air_run()

        self.assertIsNotNone(sender.started_lines)
        self.assertIn("; RouterKing rectangle pocket template: Manual Preview Pocket", widget._gcode_edit.toPlainText())
        self.assertTrue(any("X10" in line and "Y20" in line for line in sender.started_lines))
        self.assertFalse(any(line.startswith("M3") for line in sender.started_lines))
        self.assertTrue(all("Z-" not in line for line in sender.started_lines if "Z" in line))

    def test_manual_template_safe_z_is_relative_to_live_prepare_z(self):
        main_dock = _load_main_dock_module()
        status = {
            "state": "Idle",
            "MPos": "-219.325,-286.288,-10.365",
            "WPos": "-219.325,-286.288,-11.365",
            "WCO": "0.000,0.000,1.000",
        }
        widget = main_dock.RouterKingDockWidget.__new__(main_dock.RouterKingDockWidget)
        widget._sender = _FakeConnectedSender(status=status)
        ...
        widget._controller_manual_clearance = _DummySpin(1.5)
        widget._controller_manual_xyz_active = True
        widget._manual_xyz_preview_wpos = {"x": -213.325, "y": -270.288, "z": -11.365}
        widget._manual_xyz_prepare_mpos = {"x": -219.325, "y": -286.288, "z": -10.365}
        widget._manual_xyz_preview_origin_wpos = {"x": -213.325, "y": -270.288, "z": -11.365}
        widget._last_template_spec = None
        widget._template_controls = self._rectangle_template_controls()
        widget._template_controls["safe_z"].setValue(6.0)
        widget._selected_template_source = mock.Mock(return_value={})
        profile = {
            "prefer_profile_limits": True,
        ...
        with mock.patch.object(main_dock, "grbl_load_machine_profile", return_value=(profile, "/tmp/profile.json")):
            gcode, error = widget._current_stream_gcode_text(update_editor=True)

        self.assertIsNone(error)
        self.assertIn("G0 Z-9.865", gcode)
        self.assertNotIn("G0 Z6", gcode)
        self.assertEqual(widget._last_template_spec.start_z, -11.365)
        self.assertEqual(widget._last_template_spec.safe_z, -9.865)

    def test_existing_rectangle_template_stream_reanchors_to_live_position_when_manual_start_not_set(self):
        main_dock = _load_main_dock_module()
        status = {
            "state": "Idle",
            "MPos": "-245.613,-307.788,-14.699",
            "WCO": "0.000,0.000,1.000",
        }
        widget = main_dock.RouterKingDockWidget.__new__(main_dock.RouterKingDockWidget)
        widget._sender = _FakeConnectedSender(status=status)
        widget._gcode_edit = _DummyPlainTextEdit(
            "; RouterKing rectangle pocket template: stale\n"
            "; cut start target: X51.387 Y69.212\n"
            "G21\nG90\nG17\nG0 Z-10.599\nG0 X51.387 Y69.212\nG1 Z-12.699 F300\n"
        )
        widget._controller_manual_clearance = _DummySpin(1.5)
        widget._controller_manual_xyz_active = False
        ...
        self.assertIsNone(error)
        self.assertIn("; cut start target: X-245.613 Y-307.788", gcode)
        self.assertIn("G0 X-245.613 Y-307.788", gcode)
        self.assertIn("G0 Z-14.199", gcode)
        self.assertNotIn("X51.387 Y69.212", gcode)
        self.assertNotIn("G0 Z-10.599", gcode)
        self.assertEqual(widget._last_template_spec.start_z, -15.699)
        self.assertEqual(widget._last_template_spec.safe_z, -14.199)

    def test_existing_rectangle_template_stream_uses_profile_work_offset_when_status_has_only_mpos(self):
        main_dock = _load_main_dock_module()
        status = {
            "state": "Idle",
            "MPos": "-234.000,-299.237,-13.994",
        }
        widget = main_dock.RouterKingDockWidget.__new__(main_dock.RouterKingDockWidget)
        widget._sender = _FakeConnectedSender(status=status)
        widget._gcode_edit = _DummyPlainTextEdit(
            "; RouterKing rectangle pocket template: stale\n"
            "; cut start target: X63 Y77.763\n"
            "G21\nG90\nG17\nG0 Z-9.894\nG0 X63 Y77.763\nG1 Z-11.994 F300\n"
        )
        widget._controller_manual_clearance = _DummySpin(1.5)
        widget._controller_manual_xyz_active = False
        ...
        self.assertIsNone(error)
        self.assertIn("; cut start target: X-234 Y-299.237", gcode)
        self.assertIn("G0 X-234 Y-299.237", gcode)
        self.assertIn("G0 Z-13.494", gcode)
        self.assertNotIn("X63 Y77.763", gcode)
        self.assertNotIn("G0 Z-9.894", gcode)
        self.assertEqual(widget._last_template_spec.start_z, -14.994)
        self.assertEqual(widget._last_template_spec.safe_z, -13.494)

    def test_air_run_uses_live_g54_when_status_reports_stale_zero_wco(self):
        main_dock = _load_main_dock_module()
        sender = _FakeConnectedSender(
            status={
                "state": "Idle",
                "MPos": "-241.850,-333.150,-11.496",
                "WCO": "0.000,0.000,0.000",
            },
            coordinate_lines=[
        ...
        profile = {
            "prefer_profile_limits": True,
            "machine_limits": {"x": [-297.0, 103.0], "y": [-377.0, 23.0], "z": [-63.0, -3.0]},
            "max_feeds_mm_min": {"x": 2000.0, "y": 2000.0, "z": 500.0},
            "settings": {"$23": "3", "$27": "3", "$32": "0"},
            "work_offset": {"x": 0.0, "y": 0.0, "z": 1.0},
        }
        ...
        self.assertIn("$#", sender.commands)
        self.assertIsNotNone(sender.started_lines)
        self.assertTrue(any("X-91.85" in line and "Y-143.15" in line for line in sender.started_lines))
        self.assertIn("; cut start target: X-91.85 Y-143.15", widget._gcode_edit.toPlainText())

    def test_go_to_manual_start_uses_relative_safe_z_in_negative_work_coordinates(self):
        main_dock = _load_main_dock_module()
        sender = _FakeConnectedSender(
            status={
                "state": "Idle",
        ...
        widget._sender = sender
        widget._controller_manual_clearance = _DummySpin(1.5)
        widget._gcode_manual_start_wpos = {"x": -213.325, "y": -270.288, "z": -11.365}
        widget._gcode_manual_start_wco = {"x": 0.0, "y": 0.0, "z": 1.0}
        widget._append_console = mock.Mock()
        widget._request_status = mock.Mock()
        widget._explore_active = False
        profile = {
        ...
        self.assertEqual(sender.commands[:2], ["G90 G21", "G0 Z-9.865"])

    def test_air_run_button_enabled_for_manual_template_even_when_editor_is_empty(self):
        main_dock = _load_main_dock_module()
        widget = self._make_widget(main_dock, sender=_FakeConnectedSender(status={"state": "Idle"}))
        widget._explore_active = False
        widget._gcode_edit = _DummyPlainTextEdit("")
        widget._air_run_btn = _DummyWidget()
        ...
        self.assertTrue(widget._air_run_btn.enabled)
    ...
    def test_start_button_tooltip_reports_block_reason(self):
        main_dock = _load_main_dock_module()
        widget = self._make_widget(main_dock, sender=_FakeConnectedSender(status={"state": "Alarm"}))
        widget._explore_active = False
        widget._gcode_edit = _DummyPlainTextEdit("G90\nG1 X1 Y1")
        ...
        self.assertFalse(widget._start_btn.enabled)
        self.assertEqual(widget._start_btn.tooltip, "Start disabled: machine not idle (alarm).")
    ...
    def test_insert_template_aligns_to_active_manual_xyz_start_and_relative_z(self):
        main_dock = _load_main_dock_module()
        widget = main_dock.RouterKingDockWidget.__new__(main_dock.RouterKingDockWidget)
        widget._gcode_edit = _DummyPlainTextEdit("")
        widget._template_controls = self._rectangle_template_controls()
        ...
        text = widget._gcode_edit.toPlainText()
        self.assertIn("; cut start target: X-213.325 Y-270.288", text)
        self.assertIn("G0 Z-9.865", text)
        self.assertNotIn("G0 Z6", text)
        self.assertIn("G0 X-213.325 Y-270.288", text)
        self.assertEqual(widget._last_template_spec.cut_start_x, -213.325)
        self.assertEqual(widget._last_template_spec.cut_start_y, -270.288)
        self.assertEqual(widget._last_template_spec.start_z, -11.365)
        self.assertEqual(widget._last_template_spec.safe_z, -9.865)
    ...
    def test_default_rectangle_template_loads_saved_settings(self):
        ...
        self.assertTrue(spec.final_surface)
        self.assertEqual(spec.final_surface_step, 0.2)
    ...
    def test_save_rectangle_template_settings_persists_controls(self):
        ...
        widget._template_controls = {
            ...
            "final_surface": _DummyCheckBox(True),
            "final_surface_step": _DummySpin(0.2),
        }
        widget._selected_template_source = mock.Mock(return_value={})
        widget._append_console = mock.Mock()
        ...
        self.assertTrue(saved["final_surface"])
        self.assertEqual(saved["final_surface_step"], 0.2)
        widget._append_console.assert_called_once()
    ...
    def test_manual_start_alignment_is_idempotent_for_previously_aligned_template(self):
        main_dock = _load_main_dock_module()
        widget = main_dock.RouterKingDockWidget.__new__(main_dock.RouterKingDockWidget)
        spec = main_dock.rectangle_pocket_preset("tee_tablett")
        start = {"x": -228.563, "y": -290.612, "z": -12.328}

        first = widget._template_spec_aligned_to_manual_start(spec, start)
        second = widget._template_spec_aligned_to_manual_start(first, start)
        first_path = main_dock.parse_gcode_preview(main_dock.rectangle_pocket(first).gcode)
        second_path = main_dock.parse_gcode_preview(main_dock.rectangle_pocket(second).gcode)
        first_cut = next(segment.start for segment in first_path.segments if not segment.rapid)
        second_cut = next(segment.start for segment in second_path.segments if not segment.rapid)

        self.assertEqual(first.start_x, second.start_x)
        self.assertEqual(first.start_y, second.start_y)
        self.assertAlmostEqual(first_cut.x, start["x"], places=3)
        self.assertAlmostEqual(first_cut.y, start["y"], places=3)
        self.assertAlmostEqual(second_cut.x, start["x"], places=3)
        self.assertAlmostEqual(second_cut.y, start["y"], places=3)
    ...
    def test_save_manual_start_persists_manual_xyz_preview_position(self):
        main_dock = _load_main_dock_module()
        store = {}
        params = types.SimpleNamespace(
            GetString=lambda key, default="": store.get(key, default),
        ...
        widget = main_dock.RouterKingDockWidget.__new__(main_dock.RouterKingDockWidget)
        widget._sender = _FakeConnectedSender(status={"state": "Idle", "WCO": "0.000,0.000,1.000"})
        widget._controller_manual_xyz_active = True
        widget._manual_xyz_preview_wpos = {"x": -217.0, "y": -262.0, "z": -11.0}
        widget._controller_guard_mpos = {"x": -217.0, "y": -262.0, "z": -10.0}
        widget._gcode_manual_start_wpos = None
        widget._gcode_manual_start_mpos = None
        widget._gcode_manual_start_wco = None
        widget._manual_start_status = _DummyWidget()
        widget._append_console = mock.Mock()
        widget._update_preview = mock.Mock()
        ...
        with mock.patch.object(main_dock.App, "ParamGet", return_value=params):
            widget._on_save_manual_start()

        saved = json.loads(store["last_start_json"])
        self.assertEqual(saved["wpos"], {"x": -217.0, "y": -262.0, "z": -11.0})
        self.assertEqual(saved["mpos"], {"x": -217.0, "y": -262.0, "z": -10.0})
        self.assertEqual(widget._gcode_manual_start_wpos, {"x": -217.0, "y": -262.0, "z": -11.0})
        self.assertEqual(widget._manual_start_status.text, "Manual start: X-217.000 Y-262.000 Z-11.000")

    def test_load_manual_start_regenerates_template_without_machine_move(self):
        main_dock = _load_main_dock_module()
        store = {
            "last_start_json": json.dumps(
                {
                    "version": 1,
                    "saved_at": 123.0,
                    "wpos": {"x": -217.0, "y": -262.0, "z": -11.0},
                    "mpos": {"x": -217.0, "y": -262.0, "z": -10.0},
                    "wco": {"x": 0.0, "y": 0.0, "z": 1.0},
                }
            )
        }
        params = types.SimpleNamespace(
        ...
        widget = main_dock.RouterKingDockWidget.__new__(main_dock.RouterKingDockWidget)
        widget._sender = _FakeConnectedSender(status={"state": "Idle"})
        widget._controller_manual_clearance = _DummySpin(1.5)
        widget._controller_manual_xyz_active = False
        widget._manual_xyz_preview_wpos = None
        widget._gcode_manual_start_wpos = None
        ...
        with mock.patch.object(main_dock.App, "ParamGet", return_value=params):
            with mock.patch.object(main_dock, "grbl_load_machine_profile", return_value=(profile, "/tmp/profile.json")):
                widget._on_load_manual_start()

        text = widget._gcode_edit.toPlainText()
        self.assertIn("; cut start target: X-217 Y-262", text)
        self.assertIn("G0 X-217 Y-262", text)
        self.assertEqual(widget._gcode_manual_start_wpos, {"x": -217.0, "y": -262.0, "z": -11.0})
        self.assertEqual(widget._gcode_manual_start_mpos, {"x": -217.0, "y": -262.0, "z": -10.0})
        self.assertEqual(widget._gcode_manual_start_wco, {"x": 0.0, "y": 0.0, "z": 1.0})
        self.assertFalse(widget._sender.started_lines)
    ...
    def test_manual_xyz_home_matches_real_work_area_home_when_status_has_wco_without_wpos(self):
        main_dock = _load_main_dock_module()
        widget = main_dock.RouterKingDockWidget.__new__(main_dock.RouterKingDockWidget)
        ...
        self.assertAlmostEqual(home.x, -297.0)
        self.assertAlmostEqual(home.y, -377.0)
        self.assertEqual((cut_start.x, cut_start.y, cut_start.z), (-264.9, -339.475, -4.0))
    ...
    def test_manual_xyz_work_area_uses_real_work_offset_not_prepare_local_origin(self):
        main_dock = _load_main_dock_module()
    ...
    def test_current_machine_limits_corrects_stale_positive_z_profile_limits(self):
        main_dock = _load_main_dock_module()
        widget = main_dock.RouterKingDockWidget.__new__(main_dock.RouterKingDockWidget)
        widget._limits = {}
        profile = {
        ...
        self.assertEqual(limits["z"], (-63.0, -3.0))
    ...
    def test_append_controller_binding_token_maps_l2_to_z_down(self):
        main_dock = _load_main_dock_module()
        widget = main_dock.RouterKingDockWidget.__new__(main_dock.RouterKingDockWidget)
        edit = _DummyLineEdit("R2")
        widget._controller_binding_edits = {"z_axes": edit}
        widget._save_controller_defaults = mock.Mock()

        widget._append_controller_binding_token("z_axes", "L2")

        self.assertEqual(edit.text(), "R2, -L2")
        widget._save_controller_defaults.assert_called_once()

    def test_controller_binding_strings_normalizes_stale_l2_z_axis(self):
        main_dock = _load_main_dock_module()
        widget = main_dock.RouterKingDockWidget.__new__(main_dock.RouterKingDockWidget)
        widget._controller_binding_edits = {"z_axes": _DummyLineEdit("R2, L2")}

        bindings = widget._controller_binding_strings()

        self.assertEqual(bindings["z_axes"], "R2, -L2")
    ...
    def test_controller_tick_waits_for_neutral_before_sending_after_prepare(self):
        main_dock = _load_main_dock_module()
        biased_state = types.SimpleNamespace(
            name="Pad",
            x=0.0,
            y=0.0,
            z=-0.4,
        ...
        neutral_state = types.SimpleNamespace(
            name="Pad",
            x=0.0,
            y=0.0,
            z=0.0,
        ...
        widget._controller = types.SimpleNamespace(
            is_connected=lambda: True,
            poll_mapped=mock.Mock(side_effect=[biased_state, neutral_state, active_state]),
            error="",
        )
        widget._controller_feed = _DummySpin(600.0)
        widget._controller_deadzone = _DummySpin(0.2)
        ...
        widget._controller_was_active = False
        widget._controller_manual_xyz_active = True
        widget._controller_waiting_for_neutral = True
        widget._controller_status = _DummyWidget()
        widget._controller_binding_strings = mock.Mock(return_value={})
        widget._controller_fast_xy_allowed = mock.Mock(return_value=True)
        widget._send_jog = mock.Mock(return_value=True)

        with mock.patch.object(main_dock.time, "time", side_effect=[100.25, 100.50, 100.75]):
            widget._controller_tick()
            widget._controller_tick()
            widget._controller_tick()
        ...
        widget._send_jog.assert_called_once()
        kwargs = widget._send_jog.call_args.kwargs
        self.assertLess(kwargs["z"], 0.0)
        self.assertFalse(widget._controller_waiting_for_neutral)
